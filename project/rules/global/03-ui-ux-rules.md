# Global Rules — UI/UX

1. Uygulama hem web hem %100 mobil uyumlu olmalıdır.
2. Tüm ekranlar mobile-first düşünülmelidir.
3. Son kullanıcıya ERP karmaşıklığı yansıtılmamalıdır.
4. Formlar mümkün olduğunca kısa tutulmalıdır.
5. Kritik işlemler tek ekranda, az alanla tamamlanmalıdır.
6. Buton, başlık, durum etiketleri ve hata mesajları tamamen Türkçe olmalıdır.
7. İş akışı odaklı ekranlar tercih edilmelidir, veri yoğun ekranlar değil.
